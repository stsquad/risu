    ...
    while (!found) {
       switch (ctx->magic)
       {
          ...
          case EXTRA_MAGIC:
             ...
             fprintf(stderr, "%s: found EXTRA_MAGIC jumped to %p\n", __func__, ctx);
             abort();
          case 0:
...
int reginfo_dump_mismatch(struct reginfo *m, struct reginfo *a, FILE * f)
{
    ...
    if (test_sve) {
        ...
        if (!sve_preg_is_eq(&m->ffr, &a->ffr)) {
           fprintf(f, "  FFR   : ");
           sve_dump_preg_diff(f, sve_vq_from_vl(m->vl),
                              &m->pregs[i], &a->pregs[i]);
        }
        for (i = 0; i < SVE_NUM_PREGS; i++) {
           if (!sve_preg_is_eq(&m->pregs[i], &a->pregs[i])) {
              fprintf(f, "  P%2d   : ", i);
              sve_dump_preg_diff(f, sve_vq_from_vl(m->vl),
                                 &m->pregs[i], &a->pregs[i]);
           }
              ...
              for (q = 0; q < sve_vq_from_vl(m->vl); q++) {
                 if (m->zregs[i][q] != a->zregs[i][q]) {
                    fprintf(f, "%sq%02d: %016" PRIx64 "%016" PRIx64
                            ...
                            (uint64_t) (m->zregs[i][q] >> 64),
                            (uint64_t) m->zregs[i][q],
                            (uint64_t) (a->zregs[i][q] >> 64),
                            (uint64_t) a->zregs[i][q]);
                    pad = "          ";
                 }
              }
           }
