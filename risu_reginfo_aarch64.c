    ...
    if (test_sve) {
        ...
        for (i = 0; i < 32; i++) {
            fprintf(f, "  V%-2d   : %016" PRIx64 "%016" PRIx64 "\n", i,
                    (uint64_t) (ri->fp.vregs[i] >> 64),
                    (uint64_t) (ri->fp.vregs[i] & 0xffffffffffffffff));
        }
    }
...
int reginfo_dump_mismatch(struct reginfo *m, struct reginfo *a, FILE * f)
{
    ...
    if (test_sve) {
        ...
        for (i = 0; i < 32; i++) {
            if (m->fp.vregs[i] != a->fp.vregs[i]) {
                fprintf(f, "  V%-2d   : "
                        "%016" PRIx64 "%016" PRIx64 " vs "
                        "%016" PRIx64 "%016" PRIx64 "\n", i,
                        (uint64_t)(m->fp.vregs[i] >> 64),
                        (uint64_t)m->fp.vregs[i],
                        (uint64_t)(a->fp.vregs[i] >> 64),
                        (uint64_t)a->fp.vregs[i]);
            }
        }
    }
    return !ferror(f);
